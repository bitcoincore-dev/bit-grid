...
- Input is handled in the main loop with polling; `q`/`Esc` quit, arrow keys or `hjkl` move, space toggles, `r` randomizes a valid test-network secret, `f` fills the entire grid, `c` clears, `Tab` cycles testnet/testnet4/signet/regtest, and `\`/`|` toggles extra info; `?` opens the help popup.
